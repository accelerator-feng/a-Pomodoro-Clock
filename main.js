$(function() {
    ...
    function showTime() {
        ...
        secs -= 1;
        if (secs === -1 && timeMark === 1) {
            timeMark = 2;
            secs = $(".break .number").text() * 60;
            cache = null;
            origin = secs;
            $("h2").text("Break!");
            $(".background").css("background", "#f44");
        } else if (secs === -1 && timeMark === 2) {
            timeMark = 1;
            secs = $(".session .number").text() * 60;
            cache = null;
            origin = secs;
            $("h2").text("Session");
        ...
        switch (tg.attr("class")) {
            case "plus":
                if (!timer) {
                    ...
                    if (tg.attr("id") === "right_plus" && timeMark === 1) {
                        time.text(sessionTime.text());
                        secs = sessionTime.text() * 60;
                    }
                    if (tg.attr("id") === "left_plus" && timeMark === 2) {
            ...
            case "minus":
                if (!timer) {
                    ...
                    if (tg.attr("id") === "right_minus" && timeMark === 1) {
                        time.text(sessionTime.text());
                        secs = sessionTime.text() * 60;
                    }
                    if (tg.attr("id") === "left_minus" && timeMark === 2) {
                        time.text(breakTime.text());
                        secs = breakTime.text() * 60;
                    }
                    cache = null;
